...
|---|---|
| [`app.html`](frontend/app.html) | The full Dev Hub — searchable, progress, **280 pages grouped into collapsible sub-sections** (serve it; `index.html` redirects here) |
...
- **`frontend/`** — the hub app (`app.html`): 280 interactive visualizers across
  11 tracks, a searchable **two-level sidebar** (each track's pages are grouped into
...
> It maps the 11 tracks and hands you ready-made learning paths — including a featured
> *"how a CIAM app works, end to end"* walkthrough (Angular OIDC login → tokens →
> guards → Spring filter chain → method security) — so you're never staring at 280
